...
func TestPostAdHandler_Success(t *testing.T) {
	...
	expectedRequest := NewPostAdRequest(id, payload.Title, payload.Description, payload.Price)
	mockService.On("Execute", expectedRequest).Return(nil)
	router := gin.Default()
	router.PUT("/ads/:id", PostAdHandler(mockService))
...
func (m MockPostAdService) Execute(request PostAdRequest) error {
