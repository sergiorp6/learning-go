...
func (m MockFindByIdService) Execute(request FindByIdRequest) (*Ad, error) {
...
func TestFindByIdHandler_NotFound(t *testing.T) {
	...
	mockService.On("Execute", NewFindByIdRequest(expected.Id().String())).Return(nil, nil)
	router := gin.Default()
	router.GET("/:id", FindByIdHandler(mockService))
